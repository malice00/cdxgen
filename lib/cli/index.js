...
export async function createJavaBom(path, options) {
  ...
  if (
    ...
    let gradleCmd = getGradleCommand(gradleRootPath, null);
    ...
    let gradleArguments = buildGradleCommandArguments(
    ...
    if (isWin) {
      const batchFile = join(`${dirname(gradleCmd)}`, `${randomUUID()}.bat`);
      console.log(batchFile)
      temporaryFiles.add(batchFile);
      writeFileSync(batchFile, `${basename(gradleCmd)} ${gradleArguments.join(" ")}`);
      gradleCmd = batchFile;
      gradleArguments = [];
      console.log(
        "Executing",
