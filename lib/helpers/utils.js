...
export function executePodCommand(parameters, path, options) {
  if (parameters.length !== 1) {
    console.log("Executing pod", parameters[0], parameters[1]);
  } else if (path) {
    console.log("Executing pod", parameters[0], "in", path);
  } else {
    console.log("Executing pod", parameters[0]);
  }
...
export async function buildObjectForCocoaPod(
  ...
  if (
    ...
    let tmpDir = dependency.properties
      .find(({ name }) => name === "projectDir")
      .value.replaceAll("\\", "/");
...
export async function buildObjectForGradleModule(name, metadata) {
  ...
  if (["true", "1"].includes(process.env.GRADLE_RESOLVE_FROM_NODE)) {
    let tmpDir = metadata.properties
      .find(({ name }) => name === "projectDir")
      .value.replaceAll("\\", "/");
