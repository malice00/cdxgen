...
export function executeParallelGradleProperties(dir, allProjectsStr) {
  let gradleCmd = getGradleCommand(dir, null);
  let gradleArgs = buildGradleCommandArguments(
...
//    writeFileSync(scriptFile, `$result = (& ".\\${basename(gradleCmd)}" ${gradleArgs.join(" ")}) -join "\`n";return $result;`);
//    if (!isAbsolute(gradleCmd) && !gradleCmd.startsWith(".\\")) {
//      gradleCmd = `.\\${gradleCmd}`;
//    }
//    gradleArgs = ["--version"];
//    console.log(
...
//    );
  ...
  const result1 = spawnSync(gradleCmd, ["--version"], {
    cwd: dir,
    encoding: "utf-8",
    shell: isWin ? "powershell" : isWin,
    maxBuffer: MAX_BUFFER,
  });
  let stdout = result1.stdout;
  console.log(stdout)
  const result2 = spawnSync(gradleCmd, ["properties"], {
    cwd: dir,
    encoding: "utf-8",
    shell: isWin ? "powershell" : isWin,
    maxBuffer: MAX_BUFFER,
